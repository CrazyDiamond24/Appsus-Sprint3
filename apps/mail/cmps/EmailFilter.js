/*export default {
    template: `
        <section>
            <h2>Filter</h2>
            <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Quis enim rem porro delectus. Quos expedita ipsam repellendus voluptas quas, nam ea eligendi veniam ullam, modi impedit eveniet quia quaerat molestias?</p>
        </section>
    `,
}*/

export default {
    template: `
    <section>
    <button></div>Inbox <span class="inbox-count">0</span></button>
    <button></div>Starred</button>
    <button></div>Sent</button>
    <button></div>Drafts<span class="drafts-count">0</span></button>
    <button><div></div>Trash</button>
    <button></div>All</button>
    </section>
    `,
}